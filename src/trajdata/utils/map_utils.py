...
def load_rtrees(
    ...
    try:
        ...
        if exc.name != "shapely.io":
            raise
        warnings.warn(
            (
                "Cached map RTree was serialized with a newer Shapely version and "
                f"cannot be loaded from {rtrees_path}. Falling back to rebuilding "
                "the RTree from the vector map in the current environment."
            ),
            UserWarning,
        )
        return None
