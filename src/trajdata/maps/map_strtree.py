...
try:
    ...
    def linearrings(coords):
        return LinearRing(coords)

    def points(point):
        return Point(point)

    def polygons(shell, holes=None):
...
class MapElementSTRTree:
    ...
    def _build_strtree(
        ...
        self._geom_id_to_index = {id(geom): idx for idx, geom in enumerate(geometries)}
        self._geom_wkb_to_index = {geom.wkb: idx for idx, geom in enumerate(geometries)}
        return tree, np.array(ids)

    def _geometry_index(self, geom: Polygon) -> int:
        index = self._geom_id_to_index.get(id(geom))
        if index is not None:
            return index
        if not hasattr(self, "_geom_wkb_to_index"):
            geometries = getattr(
                self,
                "_geometries",
                getattr(self.strtree, "geometries", getattr(self.strtree, "_geoms", [])),
            )
            self._geom_wkb_to_index = {
                candidate.wkb: idx for idx, candidate in enumerate(geometries)
            }
        return self._geom_wkb_to_index[geom.wkb]
    ...
    def query_point(
        ...
        **kwargs,
        ...
        """
        query_point = points(point)
        if kwargs and not SHAPELY_V2:
            predicate = kwargs.pop("predicate", None)
            distance = kwargs.pop("distance", None)
            if kwargs or predicate != "dwithin" or distance is None:
                raise TypeError(
                    "Only predicate='dwithin' with distance is supported for "
                    "Shapely 1.x STRtree queries."
                )
            result = self.strtree.query(query_point.buffer(float(distance)))
            indices = np.asarray(
                [
                    self._geometry_index(geom)
                    for geom in result
                    if geom.distance(query_point) <= float(distance)
                ],
                dtype=np.intp,
            )
            return self.elem_ids[indices]

        result = self.strtree.query(query_point, **kwargs)
        if SHAPELY_V2:
            indices = np.asarray(result, dtype=np.intp)
        else:
            indices = np.asarray(
                [self._geometry_index(geom) for geom in result], dtype=np.intp
            )
        return self.elem_ids[indices]
    ...
    def nearest_area(
        ...
        **kwargs,
        ...
        """
        if kwargs and not SHAPELY_V2:
            raise TypeError("STRtree.nearest keyword arguments require Shapely 2.x")
        result = self.strtree.nearest(points(point), **kwargs)
        idx = int(result) if SHAPELY_V2 else self._geometry_index(result)
